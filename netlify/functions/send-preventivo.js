...
function buildUserEmailHtml({ nome, selectedCourses, quote }) {
  return `
    <div style="font-family: Arial, sans-serif; color: #1f1f1f; line-height: 1.6;">
      <h2 style="margin-bottom: 12px;">Richiesta ricevuta</h2>
      <p>Ciao ${escapeHtml(nome)},</p>
      <p>
        abbiamo ricevuto la tua richiesta di informazioni sui corsi SDAC.
        In allegato trovi il PDF con il riepilogo dei corsi selezionati e il preventivo indicativo.
      </p>
      <p><strong>Corsi richiesti:</strong></p>
      <ul>${buildCourseItemsHtml(selectedCourses)}</ul>
      ${buildQuoteHtml(quote)}
      <p>
        Per ulteriori chiarimenti puoi rispondere a questa email oppure contattare la segreteria.
      </p>
      <p>SDAC - Scuola D'Arte Cinematografica</p>
    </div>
  `;
}
...
function buildSuccessMessage(emailSentCount) {
  if (emailSentCount === 2) {
    return 'Richiesta inviata correttamente. Il PDF viene scaricato sul dispositivo ed è stato inviato anche via email.';
  }

  if (emailSentCount === 1) {
    return 'Richiesta completata. Il PDF viene scaricato sul dispositivo e una delle email è stata inviata correttamente.';
  }

  return "Richiesta registrata. Il PDF viene scaricato sul dispositivo; l'invio email non è ancora disponibile o non è andato a buon fine.";
}
...
export default async (req) => {
  ...
  try {
    ...
    const schoolEmail = process.env.SDAC_TO_EMAIL;

    let emailSentCount = 0;
    const warnings = [];

    if (resendApiKey && fromEmail && schoolEmail) {
      const resend = new Resend(resendApiKey);

      const userEmailResult = await resend.emails.send({
        from: fromEmail,
        to: [payload.email],
        subject: 'SDAC - Riepilogo della tua richiesta',
      ...
      if (userEmailResult.error) {
        warnings.push(userEmailResult.error.message || "Errore durante l'invio all'utente.");
      } else {
        emailSentCount += 1;
      }

      const schoolEmailResult = await resend.emails.send({
        from: fromEmail,
        to: [schoolEmail],
      ...
      if (schoolEmailResult.error) {
        warnings.push(schoolEmailResult.error.message || "Errore durante l'invio alla scuola.");
      } else {
        emailSentCount += 1;
      }
    } else {
      warnings.push('Configurazione email incompleta su Netlify.');
    }
